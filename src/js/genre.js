...
Promise.all([getApiPromise('movie', 'day'), genresListPromise()]).then(function (data) {
  ...
  filmsInfo.forEach(element => {
    ...
    item.getElementsByClassName('film__rating')[0].textContent =
      element.vote_average % 1 != 0 ? element.vote_average : element.vote_average + '.0';
    item.getElementsByClassName('film__year')[0].textContent = new Date(
